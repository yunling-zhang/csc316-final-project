...
function initCollisionChart(rawData) {
    ...
    const tooltip = d3.select('.roadtype-tooltip');
    ...
    const legend = d3.select('#roadtype-legend');
    ...
    setTimeout(() => {
        ...
        // After the circular chart is ready, build mini preview
        updateRoadtypeMiniPreview();
    }, 100);

}

function updateRoadtypeMiniPreview() {
    const mini = document.querySelector('.roadtype-screen');
    if (!mini) return;

    const src = document.querySelector('#chart svg');
    if (!src) return;
...
// On modal close
document.addEventListener('roadtype-update-mini', updateRoadtypeMiniPreview);
