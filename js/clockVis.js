...
function updateClockHands(colorScale, allTimeSegments, svg) {
    ...
    // All hour hands (main clock + mini preview)
    const hands = d3.selectAll(".hour-hand");

    // 1) Always rotate the hand to the current time
    hands
        .transition().duration(250).ease(d3.easeCubicOut)
        .attr("transform", `rotate(${hAngle})`)
        .attr("y2", -hourLen);

    // 2) Figure out which segment we're in
    const currentSegment = allTimeSegments.find(
        seg => hours >= seg.start && hours < seg.end
    );

    let inCurrentMode = false;
    if (currentSegment) {
        inCurrentMode = isDayMode
            ? (currentSegment.start >= 0 && currentSegment.start < 12)
            : (currentSegment.start >= 12 && currentSegment.start < 24);
    }

    if (currentSegment && inCurrentMode) {
        // Hand should be visible in this mode
        hands.style("opacity", 1);

        // Highlight the active segment
        d3.selectAll(".time-segment")
            .classed("highlight", d =>
                d.start === currentSegment.start && d.end === currentSegment.end
            );
        ...
        const currentTimeStr = fmtHour24(currentSegment.start);
        const riskLevelStr = riskLabel(v, maxV);

        // ⚠️ Use <br> to force two-line layout
        d3.select("#risk-banner")
            .html(
                `Current time period: ${fmtHour24(currentSegment.start)}–${fmtHour24(endHour)}
                <br>Current risk level: ${riskLevelStr}`
            )
        ...
        // Hand not applicable in this mode → hide and clear highlights / banner
        hands.style("opacity", 0);
        d3.selectAll(".time-segment").classed("highlight", false);
    }

    // Hour label outline still follows real time (optional)
...
function setClockMode(dayMode, colorScale, timeSegments, svg, bindInteractions) {
    ...
    updateClockHands(colorScale, timeSegments, svg);
}
...
// ---------- Mini preview for Panel 3 ----------
function updateClockMiniPreview() {
    // Small patch in the comic panel
    const mini = document.querySelector(".clock-screen");
    if (!mini) return;

    // All SVGs inside the clock container (legend + clock face)
    const svgs = document.querySelectorAll("#clock-visualization .clock-container svg");
    if (!svgs.length) return;

    // The *last* SVG is the clock face (legend is created earlier)
    const mainClockSvg = svgs[svgs.length - 1];

    const clone = mainClockSvg.cloneNode(true);

    // IMPORTANT: set viewBox to the original drawing size
    // (matches the clockWidth and clockHeight constants at the top of clockVis.js)
    clone.setAttribute("viewBox", `0 0 ${clockWidth} ${clockHeight}`);
    clone.setAttribute("preserveAspectRatio", "xMidYMid meet");

    // Remove fixed pixel size so it scales to the patch
...
// Initial mini once the vis is rendered
document.addEventListener("DOMContentLoaded", () => {
    // give the vis some time to draw
    setTimeout(updateClockMiniPreview, 600);
});

// When the clock modal closes, refresh the mini to the latest state
document.addEventListener("clock-update-mini", updateClockMiniPreview);
...
// Initial mini once the vis is rendered
document.addEventListener("DOMContentLoaded", () => {
    // give the vis a bit of time to draw
    setTimeout(updateClockMiniPreview, 600);
});
...
// Initial attempt after the page loads (in case the clock is already drawn)
document.addEventListener("DOMContentLoaded", () => {
    setTimeout(updateClockMiniPreview, 600);
});
